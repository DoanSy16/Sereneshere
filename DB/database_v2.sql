...
create table "search" (
	...
	FOREIGN KEY (id_login) REFERENCES users(user_id)
);
...
create or replace  function  get_profile (current_user_id INT)
  returns  table (
  	user_id INT,
  	fullname text,
  	email text,
  	...
  	day_create text,
  	gender text,
  	user_province_id text,
  	user_district_id text,
  	user_ward_id text,
  	avatar text,
  	thumb text,
  	online_last_date text,
  	count_friend INT,
  	list_friends JSONB,
  	list_image JSONB
  ...
  begin
  	...
  	u.day_create::text,
  	g.gender_name::text,
  	u.user_provinces_id ::text,
  	u.user_districts_id::text,
  	u.user_wards_id ::text,
  	u.avatar::text,
  	u.thumb::text,
  	u.online_last_date::text,
  	(select count(*) from get_friend(u.user_id))::int as count_friend,
  	(select JSONB_AGG(JSONB_BUILD_OBJECT('friends',gf))  from get_friend(u.user_id) gf) as list_friends,
  	(select JSONB_AGG(JSONB_BUILD_OBJECT('images',pi2)) from post_images pi2 inner join post p 
	 on pi2.post_id =p.post_id where p.user_id =u.user_id) as list_image
  	from 
  	users u inner join gender g
  	on u.gender_id =g.gender_id 
  	where u.user_status =true and u.ban =false and u.user_id =current_user_id;
  end;
 $$
 LANGUAGE plpgsql;
--Tạo function seach nhanh ---------------------------------------------------------------------------------
--drop function search_friend_1
create or replace  function search_friend_1(current_user_id int ,search Text)
  returns  table (
    id INT,
    avatar VARCHAR(1000),
    fullname VARCHAR(255),
  ...
  declare
  search_list INT[];
  search_list_temp INT[];
  friends_list INT[];
  begin 
	SELECT ARRAY_AGG(frl.user_id)
    INTO friends_list
    FROM get_friend_follow(current_user_id) frl where frl.fullname ilike '%'|| search ||'%';
   
    select ARRAY_AGG(u_s.user_id)
    into search_list_temp 
    from users u_s 
    INNER JOIN user_role ur ON u_s.user_id = ur.user_id
    where u_s.fullname ilike '%'|| search || '%' and u_s.ban = false and u_s.user_status =true and ur.role_id = 4;
   
     SELECT ARRAY_CAT(friends_list,search_list_temp) INTO search_list;
    
    if array_length(search_list ,1)>0 then 
    return query
    select 
    	u.user_id as id,
    	u.avatar,
    	u.fullname,
    	(select JSONB_AGG(JSONB_BUILD_OBJECT('avatar', g.avatar)) from get_friend_follow(u.user_id) g where g.user_id = ANY(friends_list)) as mutual_friends
   	from users u where u.user_id =any(search_list) limit 5;
    end if;
